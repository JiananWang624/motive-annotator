...
void PrintDataDescriptions(sDataDescriptions* pDataDefs)
{
    ...
    for (int i = 0; i < pDataDefs->nDataDescriptions; i++)
    {
        ...
        else if (pDataDefs->arrDataDescriptions[i].type == Descriptor_GPIO)
        {
            ...
            printf("GPIO Name : %s\n", pGPIO->szName);
			printf("GPIO ID   : %d\n", pGPIO->ID);
			printf("GPIO Port Count : %hu\n", pGPIO->numberOfGPIOPorts);
            for (uint8_t iPort = 0; iPort < pGPIO->numberOfGPIOPorts; iPort++)
            {
				printf("\tGPIO Port %d : %s\n", iPort, pGPIO->szGPIONames[iPort]);
            }
        }
        else if (pDataDefs->arrDataDescriptions[i].type == Descriptor_Anchor)
        {
            ...
            printf("Anchor ID: %d \n", pAnchor->ActiveID);
            printf("Anchor Positions:  x: %3.2f, y: %3.2f, z%3.2f\n", pAnchor->x, pAnchor->y, pAnchor->z);
        }
        else
        {
            // Unknown
