...
namespace SampleClientML
{
    public class SampleClientML
    {
        ...
        private static List<Anchor> mAnchors = new List<Anchor>();
        ...
        /* Switches into Unicast for a final attempt before it goes to failure */
        static void tryUnicast()
        {

            string strLocalIP = "127.0.0.1";   // Local IP address (string)
            string strServerIP = "127.0.0.1";  // Server IP address (string)
            NatNetML.ConnectionType connectionType = ConnectionType.Unicast;
        ...
        static void parseDataDescriptor(List<NatNetML.DataDescriptor> description)
        {
            ...
            for (int i = 0; i < numDataSet; ++i)
            {
                ...
                switch (dataSetType)
                {
                    ...
                    case ((int)NatNetML.DataDescriptorType.eAnchorData):
                    {
                        NatNetML.Anchor anchor = (NatNetML.Anchor)description[i];
                        Console.WriteLine("\tAnchor Name: ({0})\n", anchor.Name);
                        Console.WriteLine("\tAnchor ID: ({0})\n", anchor.ID);
                        Console.WriteLine("\tAnchor Positions: [({0}), ({1}), ({2})]\n", anchor.x, anchor.y, anchor.z);
                        mAnchors.Add(anchor);
                        break;
                    }
