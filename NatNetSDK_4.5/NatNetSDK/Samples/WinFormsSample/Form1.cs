...
namespace WinFormTestApp
{
    ...
    public partial class Form1 : Form
    {
        ...
        List<IMU> mIMU = new List<IMU>();
        Hashtable htIMUs = new Hashtable();
        List<GPIO> mGPIOs = new List<GPIO>();
        Hashtable htGPIOs = new Hashtable();
        ...
        private void UpdateDataGrid()
        {
            ...
            if (htIMUs.Count > 0)
            {
                for (int i = 0; i < m_FrameOfData.nIMUs; i++)
                {
                    NatNetML.IMUData imu = m_FrameOfData.IMUs[i];
                    uint id = (uint)imu.ID;
                    int key = id.GetHashCode();
                    if (htIMUs.ContainsKey(key))
                    {
                        int rowIndex = (int)htIMUs[key];
                        if (rowIndex >= 0)
                        {
                            // Convert quaternion to eulers.  Motive coordinate conventions: X(Pitch), Y(Yaw), Z(Roll), Relative, RHS
                            float[] quat = new float[4] { imu.qx, imu.qy, imu.qz, imu.qw };
                            float[] eulers = new float[3];
                            eulers = NatNetClientML.QuatToEuler(quat, NATEulerOrder.NAT_XYZr);
                            double xRot = RadiansToDegrees(eulers[0]);     // convert to degrees
                            double yRot = RadiansToDegrees(eulers[1]);
                            double zRot = RadiansToDegrees(eulers[2]);

                            try
                            {
                                dataGridView1.Rows[rowIndex].Cells[1].Value = imu.x;
                                dataGridView1.Rows[rowIndex].Cells[2].Value = imu.y;
                                dataGridView1.Rows[rowIndex].Cells[3].Value = imu.z;
                                dataGridView1.Rows[rowIndex].Cells[4].Value = xRot;// imu.qx;
                                dataGridView1.Rows[rowIndex].Cells[5].Value = yRot;// imu.qy;
                                dataGridView1.Rows[rowIndex].Cells[6].Value = zRot;// imu.qz;
                                //dataGridView1.Rows[rowIndex].Cells[7].Value = imu.qw;
                            }
                            catch (Exception e)
                            {
                                // ok - likely cells is out of range
            ...
            if (htGPIOs.Count > 0)
            {
                for (int i = 0; i < m_FrameOfData.nGPIOs; i++)
                {
                    NatNetML.GPIOData gpio = m_FrameOfData.GPIOs[i];
                    uint id = (uint)gpio.ID;
                    int key = id.GetHashCode();
                    if (htGPIOs.ContainsKey(key))
                    {
                        int rowIndex = (int)htGPIOs[key];
                        if (rowIndex >= 0)
                        {
                            int nPorts = Math.Min(dataGridView1.Rows[rowIndex].Cells.Count, gpio.nPorts);
                            for (int iPort = 0; iPort < nPorts; iPort++)
                            {
                                try
                                {
                                    dataGridView1.Rows[rowIndex].Cells[iPort + 1].Value = gpio.PortData[iPort];
                                }
                                catch (Exception e)
                                {
                                    // ok - likely cells is out of range
        ...
        private void GetDataDescriptions()
        {
            ...
            if (bSuccess)
            {
                ...
                foreach (NatNetML.DataDescriptor d in descs)
                {
                    ...
                    // IMUs
                    else if (d.type == (int)NatNetML.DataDescriptorType.eIMUData)
                    {
                        NatNetML.IMU imu = (NatNetML.IMU)d;

                        OutputMessage("Data Def " + iObject.ToString() + " [IMU]");
                        OutputMessage(" Name : " + imu.Name);
                        OutputMessage(" ID : " + imu.ID);
                        OutputMessage(" Sensor Fused : " + imu.SensorFused.ToString());
                        OutputMessage(" Fused RigidBodyID : " + imu.RigidBodyID.ToString());
                        mIMU.Add(imu);

                        int rowIndex = dataGridView1.Rows.Add("IMU: " + imu.Name);
                        uint id = (uint)imu.ID;
                        int key = id.GetHashCode();
                        if (htIMUs.ContainsKey(key))
                            MessageBox.Show("Duplicate IMU ID");
                        else
                            htIMUs.Add(key, rowIndex);

                    }
                    // GPIOs
                    else if (d.type == (int)NatNetML.DataDescriptorType.eGPIOData)
                    {
                        NatNetML.GPIO gpio = (NatNetML.GPIO)d;

                        OutputMessage("Data Def " + iObject.ToString() + " [GPIO]");
                        OutputMessage(" Name : " + gpio.Name);
                        OutputMessage(" ID : " + gpio.ID);
                        for (int i = 0; i < gpio.PortCount; i++)
                        {
                            OutputMessage("  " + gpio.PortNames[i]);
                        }
                        mGPIOs.Add(gpio);

                        int rowIndex = dataGridView1.Rows.Add("GPIO: " + gpio.Name);
                        uint id = (uint)gpio.ID;
                        int key = id.GetHashCode();
                        if (htGPIOs.ContainsKey(key))
                            MessageBox.Show("Duplicate GPIO ID");
                        else
                            htGPIOs.Add(key, rowIndex);

                    }

                    // Anchors
                    else if (d.type == (int)NatNetML.DataDescriptorType.eAnchorData)
                    {
                        NatNetML.Anchor anchor = (NatNetML.Anchor)d;
                        OutputMessage("Data Def " + iObject.ToString() + " [Anchor]");
                        OutputMessage(" Name : " + anchor.Name);
                        OutputMessage(" ID : " + anchor.ID);
                        String anchPos = String.Format(" Position : {0}, {1}, {2}", anchor.x, anchor.y, anchor.z);
                        OutputMessage(anchPos);
                    }
                    // Assets
                    else if (d.type == (int)NatNetML.DataDescriptorType.eAssetData)
                    {
